  ...
  return(
    <div className="background !w-fit p-4 flex flex-col space-y-4">
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-2xl  text-text-light">{user?.displayName}</h1>
          <h2 className="text-base text-text">{user?.firstName} {user?.lastName}</h2>
        </div>
      </div>
    <div className="h-0.5 w-full flex">
    <div className="w-1/2 h-full bg-gradient-to-l from-50% from-primary/50"></div>
      <div className="w-1/2 h-full bg-gradient-to-r from-50% from-primary/50"></div>
    </div>
