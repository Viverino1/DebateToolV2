...
export default function ExpandedCardTemplate(props: {children: ReactNode, card: AnyCard}){
  ...
  return(
    <div className="relative w-full h-full whitespace-pre-wrap">
      ...
      <div className="absolute top-0 left-0 right-0 bottom-0 z-20 backdrop-blur-[250px] p-4 overflow-auto">
        <h3 className={`text-base mb-2 py-1 px-2 w-fit h-fit text-text-light rounded ${color.bg} font-bold`}>{capitalize(type)} Card</h3>
        <h3 className="text-base">{contSub? contSubToString(contention, subpoint) : null}</h3>
        <h1 className="text-3xl text-text-light font-bold">{title}</h1>
        <a className={`text-xl underline ${color.text}`} href={sourceLink} target="_blank">{sourceName}</a>

        <div className="mt-8">{props.children}</div>

        <h2 className="text-xl text-text-light mt-8 font-bold">Data</h2>
        <p className="w-3/4">"{data}"</p>

        <h2 className="text-xl text-text-light mt-8 font-bold">Warrant</h2>
        <p className="w-3/4">{warrant}</p>

        <h2 className="text-xl text-text-light mt-8 font-bold">Details</h2>
        ...
        <h2 className="text-xl text-text-light mt-8 font-bold">Options</h2>
        ...
        <h2 className="text-xl text-text-light mt-8 mb-2 font-bold">Creator</h2>
