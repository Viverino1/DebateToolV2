...
export default function CreateQuotePage(props: {editCard?: Quote}){
  ...
  const [card, setCard] = useState<Quote>(props.editCard?? {
    type: "quote",
    cardID: "",
    ownerUID: user.uid,
    school: user.school,
    ...
    <div className="relative w-full h-full flex">
      ...
      <div className="w-full md:w-1/2 lg:w-2/3 h-full p-4 flex flex-col space-y-4">
        <div className="flex flex-col space-y-4">
          ...
          type="text" 
          className="input !text-lg text-quote" 
          ...
          onChange={e => {
            setCard(old => ({...old, quotee: e.target.value}))
          }}
          />
        </div>
        ...
        placeholder={`Data (quote${!card.data? ` from ${card.quotee}` : ""})`}
        onChange={e => {
      ...
      <div className="relative hidden md:flex flex-col items-center justify-center md:w-1/2 lg:w-1/3 h-full p-4 pl-0">
        ...
        <div className="flex flex-col space-y-4 mt-4">
          ...
          />
          <button className="button-quote !w-32 text-text-light" onClick={() => {
